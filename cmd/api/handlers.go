...
func (app *application) deletePlantHandler(w http.ResponseWriter, r *http.Request) {
	...
	id := r.URL.Path[len("/v1/plant/"):]
	idInt, err := strconv.ParseInt(id, 10, 64)
	if err != nil {
		http.Error(w, "Bad Request", http.StatusBadRequest)
	}

	err = app.profiles.Plants.Delete(idInt)
	if err != nil {
		switch {
	...
	js, err := json.MarshalIndent(envelope{"message": "plant successfully deleted", "id": id}, "", "\t")
	if err != nil {
	...
	_, err = w.Write(js)
	if err != nil {
		fmt.Println("Error writing the DELETE plants response.")
		http.Error(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
		return
	}
}
